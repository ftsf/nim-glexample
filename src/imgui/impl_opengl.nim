...
void main() {
  ...
  var vertex_shader_source = allocCStringArray([
    $vertex_shader_glsl
  ])

  glShaderSource(gVertHandle, 1, vertex_shader_source, nil)
  glCompileShader(gVertHandle)
  igOpenGL3CheckShader(gVertHandle, "vertex shader")

  gFragHandle = glCreateShader(GL_FRAGMENT_SHADER)

  var fragment_shader_source = allocCStringArray([
    $fragment_shader_glsl
  ])

  glShaderSource(gFragHandle, 1, fragment_shader_source, nil)
